...
module.exports = function resizeFileImageWithCanvas (file, params = {}) {
  if (
    (params?.width && !typeof params?.width === "number" && params?.width <= 0) ||
    (params?.height && !typeof params?.height === "number" && params?.height <= 0) ||
    (params?.quality && !typeof params?.quality === "number" && (params?.quality < 0 || params?.quality > 1))
  )
    throw new Error("width and height must be positive integers, and quality must be a decimal with values into 0 and 1")
  let b64;
